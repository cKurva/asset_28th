  ...
  <div class="centerContainer">
    <div class="timelineContainer d-flex w-100 justify-content-end">
      ...
      <!-- <b-dropdown text="Dates" dropleft variant="dark">
        ...
        <div class="timelineDropdown mt-3" v-if="site_type === 'Construction_Site'">
      ...
      <div  v-if="site_type === 'Solar_Site'" >
        <div id="app">
          <b-dropdown id="dropdown-1" right :text="currentDate" class="select-date">
            <b-dropdown-item
            ...
            >
              ...
              >
                <b-dropdown-item
                  ...
                  :key="index"
                  @click="switchDateFromDropDown(item.title,item.type)"
                  :class="{
...
var qmpm = [];
     for (var m in this.$store.state.projectStore.datesArray) {
      ...
      if (this.$store.state.projectStore.datesArray[m].type === "SCPM") {
        scpm.push({ title: this.$store.state.projectStore.datesArray[m].date,
                    type: this.$store.state.projectStore.datesArray[m].type});
      }
      if (this.$store.state.projectStore.datesArray[m].type === "SCQM") {
                  // console.log(this.$store.state.projectStore.datesArray[m].type)
  ...
  computed: {
    ...
    currentDate: function () {
      let currentDate = this.$store.state.airmapStore.currentDate;
            this.$store.dispatch("projectStore/project_type", this.$store.state.projectStore.currentDateProject.type);
  ...
  methods: {
    ...
    switchDateFromDropDown(date,ty) {
      if (date) {
        this.$store.dispatch("projectStore/switchProjectbyDate", date);
        this.$store.dispatch("projectStore/project_type", ty );
...
.timelineContainer  button{
    /* background-color: #0e76a8 !important; */
      background-color : #170e61 !important;
...
.centerContainer .text-center button{
border-radius:0 !important;
}
.sub-menu {
  position: absolute;
