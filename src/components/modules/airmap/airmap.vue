<template>
  <div style="height: 100vh; width: 100%">
    ...
    >
      ...
      <l-control position="topright" class="project-name ml-5">
     <div
          id="dsmSwitch"
    ...
    <b-modal id="csvUploadModel" title="Upload CSV File" :hide-footer="true">
      <csvUploadModel />
    </b-modal>
     <b-modal id="csvInvUploadModel" title="Upload CSV File" :hide-footer="true">
      <csvInvUploadModel />
    </b-modal>
...
import csvUploadModel from "./csvUploadmodel.vue";
import csvInvUploadModel from "./csvInvUploadmodel.vue";
...
export default {
  ...
  computed: {
    ...mapState({
       projectName: (state) => state.projectStore.projectName,
  ...
  methods: {
    ...
    renderGeoData(url, featureLayer, kmlColor = "", hex, kml_type) {
      if (url) {
        let fileFormat = url.split(".").slice(-1)[0];
        let dataLayer = null;
        let customLayer = null;

        // console.log(kml_type);

        if (gb_layer !== null) {
          this.map.removeLayer(gb_layer);
        }
        ...
        //       gb_layer = new L.KML(kml);
        //       // Add track to map
        //       this.map.addLayer(gb_layer);
        //       const bounds = gb_layer.getBounds();
