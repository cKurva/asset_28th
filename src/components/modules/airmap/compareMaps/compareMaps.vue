<template>
  <div style="height: 100vh; width: 100%">
    ...
    <div class="timelineContainer" style="display:none">
      ...
      <div>
        <b-button pill class="btn-dark">{{ leftDate }}</b-button>
      </div>
      <div>
        <b-button pill class="btn-dark">{{ rightDate }}</b-button>
      </div>
    ...
    <div class="compare-page-btns">
      <b-button @click="showCompareDetails" variant="dark" v-b-tooltip.hover.left
        title="View Report">
      ...
      <b-button
        ...
        title="Download CSV"><i class="fas fa-download" aria-hidden="true"></i
      ></b-button>
    </div>
    ...
    >
       <l-tile-layer
        ...
        :options="{ maxZoom: 23 }"
        ref="kml_layer"
        :url="'https://takvaviyacloud.com/2k19/SCPM_450/R2/030719/kml/summary/modules_complete_1_global.kml'"
      ></l-tile-layer>
...
export default {
    ...
    datesArray: function () {
      ...
      if (this.$store.state.projectStore.projectType === "Construction_Site") {
        ...
        for (var i in this.$store.state.projectStore.datesArray) {
          if(this.$store.state.projectStore.type_pro == 'SCPM'){
            if(this.$store.state.projectStore.datesArray[i].type === 'SCPM'){
                  dates_array.push(this.$store.state.projectStore.datesArray[i].date);

            }
          }
          else if(this.$store.state.projectStore.type_pro == 'SCQM'){
            if(this.$store.state.projectStore.datesArray[i].type === 'SCQM'){
                  dates_array.push(this.$store.state.projectStore.datesArray[i].date);

            }
          }
          else if(this.$store.state.projectStore.type_pro == 'SCPM+SCQM'){
            if(this.$store.state.projectStore.datesArray[i].type === 'SCPM+SCQM'){
                  dates_array.push(this.$store.state.projectStore.datesArray[i].date);
  ...
  methods: {
    ...
    addCompareControl() {
      ...
      this.kml_layer = this.$refs.kml_layer.mapObject;
      ...
      console.log(this.baseLayerLeft)
      console.log(this.rgbLayerLeft)
      console.log(this.dsmLayerLeft)
      fetch('https://takvaviyacloud.com/2k19/SCPM_450/R2/030719/kml/summary/modules_complete_1_global.kml')
                .then(res => res.text())
                .then(kmltext => {
                    ...
                    // this.baseLayerLeft.addLayer(track);
                    console.log(track)
                    // this.kml_layer.addLayer(track); 
                });
      this.compareControl = L.control
        .sideBySide(
          [this.baseLayerLeft, this.rgbLayerLeft, this.dsmLayerLeft,this.kml_layer],
...
.timelineContainer button,.actionButtons1 button{
