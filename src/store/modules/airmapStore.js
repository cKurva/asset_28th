...
const actions = {
  ...
  csvUploadApi({ state, dispatch, rootState }, cadUploadData) {
    return new Promise((resolve, reject) => {
      if (cadUploadData) {
        // console.log(cadUploadData.cadFile)
        dispatch('toggleSpinner',null, { root: true })
        httpClient.post(`http://takvaviya.in:8001/Asset/projectdata/uploadsummary/${rootState.projectStore.projectId}/${state.currentDate}/`, cadUploadData.cadFile, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
  ...
  csvInvUploadApi({ state, dispatch, rootState }, cadUploadData) {
    return new Promise((resolve, reject) => {
      if (cadUploadData) {
        console.log(cadUploadData.cadFile)
        dispatch('toggleSpinner',null, { root: true })
        httpClient.post(`http://takvaviya.in:8001/Asset/projectdata/uploadinverter/${rootState.projectStore.projectId}/${state.currentDate}/`, cadUploadData.cadFile, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
